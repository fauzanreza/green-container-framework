...
- **Dynamic Target Management:** Layer 1 now supports a **whitelist-based scope model**
  ...
  - **Whitelist mode** (recommended for production): if `targets.json` contains a
    non-empty list of container names, HECF only monitors and shapes those containers.
    All other running containers are completely ignored — no freezing, no throttling.
  - **Open mode** (default): if `targets.json` is empty (`[]`), HECF manages all
    non-excluded containers as before.
  - On every polling cycle, Layer 1 writes `discovered_containers.json` — a snapshot
    of all currently running non-excluded containers — which the Dashboard reads to
    populate the "Managed Containers" UI panel. Users click container cards to add or
    remove them from the whitelist without any restart or config file edits.
...
- **Expanded Exclusion List:** `EXCLUDED_CONTAINERS` in `config.py` enumerates all
  known critical infrastructure, tunnel, database, and monitoring containers by name
  as a hard safety-net, independent of the whitelist mode.
...
Independent **Gunicorn/Flask** server (port 8092), using 2 workers × 4 threads to
handle concurrent browser requests without blocking — preventing HTTP 524 gateway
timeouts under load. Ingests `metrics.csv` generated by the HECF core loop.

Visualizes **exactly 5 metrics**: CPU Utilization, Memory/RAM Usage, Energy
Consumption, Web Latency (SLA), Framework Overhead. No historical database —
live aggregation only.

**Managed Containers UI Panel** (new): a visual card-based interface rendered above
...
All changes persist to `targets.json` via `POST /api/targets` and take effect in
the next polling cycle (≤30 seconds), with no container or framework restart required.
...
- **Volume Mounts (HECF engine):**
  ...
  - `./targets.json` — read/write, managed container whitelist (UI-editable)
  - `./discovered_containers.json` — read/write, container discovery snapshot for dashboard
  - `./priority_map.json` — read/write, per-container priority overrides
- **Volume Mounts (dashboard):**
  - `./metrics.csv` — read-only, live metrics from HECF engine
  - `./targets.json` — read/write, whitelist management from UI
  - `./discovered_containers.json` — read-only, container list written by engine
  - `./priority_map.json` — read/write, priority control from UI
  - `/proc/stat`, `/proc/loadavg`, `/proc/uptime` — read-only, host system stats for htop panel
...
├── requirements.txt             # numpy, docker, flask, gunicorn, requests, locust
├── dashboard.py                 # Gunicorn/Flask app — 5 metrics + managed targets UI (port 8092)
├── targets.json                 # UI-editable managed container whitelist (shared volume)
├── discovered_containers.json   # Written by HECF engine each cycle; read by dashboard UI
├── priority_map.json            # UI-editable per-container priority overrides
│
├── framework/                   # Core HECF Logic Module
│   ├── __init__.py
│   ├── config.py                # Centralized thresholds/constants + expanded exclusion list
│   ├── profiler.py              # Layer 1: discovery + targets.json filter + discovered_containers.json writer
...
| 4 | Micro-Freeze thaw is polling-based (10–30s) without eBPF. Production apps can experience up to 30s delay on first request if frozen during idle. | Use the Dashboard "Managed Containers" panel to add only experiment/test containers to the whitelist. Never add production web apps unless eBPF is available. |
