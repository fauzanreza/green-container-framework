...
1. **Latency Average** `[ms]` - H2 (RQ2)
2. **Latency p95** `[ms]` - H2 (RQ2)
3. **Throughput** `[req/s]` - H2 (RQ2)
4. **Error Rate** `[%]` - H2 (RQ2)
5. **Container Restart** `[count]` - H2 (RQ2)

### Pilar 3: Energy & Efficiency

1. **Power Consumption (mean)** `[Watt]` - H1 (RQ2)
2. **Energy Consumption (total)** `[kWh]` - H1 (RQ2)
3. **Idle Resource Waste** `[%]` - H1 (RQ2)
4. **Performance-per-Watt (PPW)** `[req/J]` - H1 (RQ2)

### Pilar 4: Environmental Impact

1. **Estimated CO₂e** `[gCO₂]` - H1 (RQ2)
2. **Carbon per Task (CIT)** `[g/req]` - H1 (RQ2)
3. **Thermal Stability** `[°C var]` - H2 (RQ2)

### Pilar 5: Meta-Metrics

1. **Framework Overhead** `[% CPU & RAM]` - H2 (RQ1)
